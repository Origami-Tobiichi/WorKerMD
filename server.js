...
function clearSessionFiles() {
    return new Promise((resolve, reject) => {
        const cmd = process.platform === 'win32' 
            ? 'rmdir /s /q nazedev 2>nul & del baileys_store.json 2>nul & del session 2>nul & del sessions 2>nul'
            : 'rm -rf ./nazedev/* ./baileys_store.json ./session ./sessions';
...
app.post('/api/advanced-fix', (req, res) => {
    ...
    const cmd = process.platform === 'win32'
        ? 'rmdir /s /q nazedev 2>nul & del baileys_store.json 2>nul & del session 2>nul & del sessions 2>nul & rmdir /s /q tmp 2>nul'
        : 'rm -rf ./nazedev ./baileys_store.json ./session ./sessions ./tmp';
...
app.post('/api/clear-and-restart', (req, res) => {
    ...
    const cmd = process.platform === 'win32'
        ? 'rmdir /s /q nazedev 2>nul & del baileys_store.json 2>nul & taskkill /f /im node.exe 2>nul'
        : 'rm -rf ./nazedev ./baileys_store.json && pkill -f "node.*index.js"';
