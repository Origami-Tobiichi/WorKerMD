...
// ⭐⭐ PERBAIKAN: Function to clear session files dengan lebih komprehensif
function clearSessionFiles() {
    return new Promise((resolve, reject) => {
        ...
        if (process.platform === 'win32') {
            commands.push(
                'rmdir /s /q nazedev 2>nul',
                'del baileys_store.json 2>nul',
                'del session.json 2>nul',
                'del sessions.json 2>nul',
                'rmdir /s /q baileys 2>nul',
                'rmdir /s /q tmp 2>nul'
            );
        } else {
            commands.push(
        ...
        commands.forEach(cmd => {
            exec(cmd, (error) => {
                completed++;
                if (error) {
                    console.log(chalk.gray(`   ⚠️ ${cmd.split(' ')[0]} completed with warnings`));
                } else {
                ...
                if (completed === totalCommands) {
                    if (hasError) {
                        reject(new Error('Some session files could not be cleared'));
                    } else {
